
荷蘭黃金時代寫實主義
簡潔的構圖與神秘的氛圍
光線柔和地融入整個畫面，營造出靜謐和諧的永恆感

生成有雞蛋花花朵、葡萄的圖片
- 風格：荷蘭黃金時代寫實主義
- 質地極為細膩寫實
- 氛圍寧靜
- 光線柔和地融入整個畫面，營造出靜謐和諧的永恆感
- 上面使用深色背景
- 下方繪出油畫風格文字：Gimbo Geisha，背後條狀背景也使用深色


核心提示詞：
主體 (Subject)：A bouquet of orange and white lilies (一束橘色與白色的百合花)
風格 (Style)：Vintage botanical illustration (復古植物插畫), Digital painting (數位繪畫), Fine art (美術), Dutch golden age flower painting (荷蘭黃金時代花卉畫)
背景 (Background)：Dark background (深色背景), Textured background (有紋理的背景), Muted dark blue background (柔和的深藍色背景)
光影 (Lighting)：Soft lighting (柔和光線), Dramatic lighting (戲劇性光線), Chiaroscuro (明暗對照法)
色彩 (Color Palette)：Harmonious color palette of orange, white, and dark grey (橘色、白色與深灰色的和諧色調)
構圖 (Composition)：Centered composition (置中構图), Elegant (優雅的)

生成有雞蛋花花朵、葡萄的圖片
- 雞蛋花外部乳白、內部黃色，以及暗紅色雞蛋花
- 還有一些葡萄
- 風格：荷蘭黃金時代花卉畫
- 質地極為細膩寫實
- 光影：Chiaroscuro (明暗對照法)
- 構圖：Elegant (優雅的)
-下方文字為：Gimbo Geisha
-----------------------------------
核心提示詞 (中文翻譯)
主體：一束橘色與白色的百合花
風格：復古植物插畫, 數位繪畫, 美術, 荷蘭黃金時代花卉畫
背景：深色背景, 有紋理的背景, 柔和的深藍色背景
光影：柔和光線, 戲劇性光線, 明暗對照法
色彩：橘色、白色與深灰色的和諧色調
構圖：置中構圖, 優雅的
提示詞組合範例 (中文翻譯)
範例 1 (綜合型):

英文: A beautiful digital painting of a bouquet of orange and white lilies, in the style of vintage botanical illustration, dramatic lighting, dark textured background, elegant and detailed.
中文: 一幅精美的數位繪畫，描繪一束橘色與白色的百合花，復古植物插畫風格，戲劇性光線，深色紋理背景，優雅且細緻。
範例 2 (強調復古感):

英文: Vintage botanical art of a floral arrangement with orange and white lilies, inspired by Dutch golden age paintings, soft chiaroscuro lighting, on a dark, moody background.
中文: 復古植物藝術，一場有橘色與白色百合花的花卉佈置，靈感來自荷蘭黃金時代畫作，柔和的明暗對照法光線，深色、有氛圍的背景。
範例 3 (簡潔版):

英文: A bouquet of orange and white flowers, fine art, digital illustration, centered composition, dark background, soft and dramatic light.
中文: 一束橘色與白色的花，美術風格，數位插畫，置中構圖，深色背景，柔和且戲劇性的光線。
範例 4 (加入封面設計元素):

英文: An elegant bouquet of orange and white lilies, vintage botanical illustration style, centered on a dark textured background, with a solid color block and classic serif text at the bottom, album cover art.
中文: 一束優雅的橘色與白色百合花，復古植物插畫風格，置於深色紋理背景中央，底部有色塊和古典襯線字體，專輯封面藝術。


--
主體：山地尼亞拉羚 (Mountain Nyala)在高海拔叢林

風格：荷蘭黃金時代寫實主義
質地極為細膩寫實
光影：Chiaroscuro (明暗對照法)
構圖：Elegant (優雅的)
----

Generate A Coffee Bean Label:
The size and ratio must refer the source image.

A minimalist pencil sketch portrait of a Guji Oromo man. He features the traditional Gutoo hairstyle (an intricate braided bun on top of the head) and is adorned with white cowrie shell decorations on a headband and necklace. He is gently holding a traditional Kirar (Ethiopian five-string lyre). The art style is a rough charcoal drawing with clean lines and graphite shading. High contrast, white background, heavy use of negative space. Artistic, cultural, and elegant.

At the bottom, the text "Nensebo Queen Berry G1" is displayed on a dark background.

Generate A Coffee Bean Label:
The size and ratio must refer the source image.

A minimalist pencil sketch portrait of a Guji Oromo man. He features the traditional Gutoo hairstyle (an intricate braided bun on top of the head) and is adorned with white cowrie shell decorations on a headband and necklace. The art style is a rough charcoal drawing with clean lines and graphite shading. High contrast, white background, heavy use of negative space. Artistic, cultural, and elegant.

At the bottom, the text "Nensebo Queen Berry G1" is displayed on a dark background.

Sidama Arbegona
-----
Generate A Coffee Bean Label:
The size and ratio must refer the source image.

A minimalist pencil sketch of a Sidamo Lark. The bird is depicted standing naturally, capturing its small crest and textured plumage. The art style is a rough charcoal drawing with clean lines and graphite shading. High contrast, white background, heavy use of negative space. Artistic, naturalistic, and elegant.

At the bottom, the text "Sidama Arbegona Natural G1" is displayed on a different dark background.
-----
Generate A Coffee Bean Label:
The size and ratio must refer the source image.

主體：包含葉子的苿莉花

- 風格：荷蘭黃金時代寫實主義
- 質地極為細膩寫實
- 光影：Chiaroscuro (明暗對照法)
- 構圖：Elegant (優雅的)

At the bottom, the text " Kersa Gelana Geisha Washed G1" is displayed on a different dark background.
----
素描風格
Generate A Coffee Bean Label:
The size and ratio must refer the source image.

A minimalist pencil sketch of a Sidamo Lark. The bird is depicted standing naturally, capturing its small crest and textured plumage. The art style is a rough charcoal drawing with clean lines and graphite shading. High contrast, white background, heavy use of negative space. Artistic, naturalistic, and elegant.

At the bottom, the text "Sidama Arbegona Natural G1" is displayed on a different dark background.
----
素描風格 參數
Generate A Coffee Bean Label:
The size and ratio must refer the source image.

A minimalist pencil sketch of a #Subject. The #Subject is depicted  naturally, capturing its small crest and textured plumage. The art style is a rough charcoal drawing with clean lines and graphite shading. High contrast, white background, heavy use of negative space. Artistic, naturalistic, and elegant.

At the bottom, the text  #Text is displayed on a different dark background.
-
#Subject: Black Rhino
#Text: Kenya Kirinyaga Kamwangi AA TOP Washed 